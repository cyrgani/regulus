...
functions! {
    ...
    "||"(2) => |state, args| Ok(Atom::Bool(
        args.at(0).eval(state)?.bool()? ||
        args.at(1).eval(state)?.bool()?
    ))
    ...
    "!"(1) => |state, args| Ok(Atom::Bool(!args.at(0).eval(state)?.bool()?))
    ...
    "^"(2) => |state, args| Ok(Atom::Int(args.at(0).eval(state)?.int()? ^ args.at(1).eval(state)?.int()?))
}
